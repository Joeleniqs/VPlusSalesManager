...
{
    internal class VPlusSalesManagerRepository<T> : IVPlusSalesManagerRepository<T> where T : class
    {
         private readonly IVPlusSalesManagerUoWork _context;
		 private readonly DbSet<T> _dbSet;
		 private readonly DbContext _dbContext;

         public VPlusSalesManagerRepository(IVPlusSalesManagerUoWork uoWork)
		{
		    _context = uoWork ?? throw new ArgumentNullException(nameof(uoWork));
			 _dbSet = uoWork.Context.VPlusSalesManagerDbContext.Set<T>();
			 _dbContext = uoWork.Context.VPlusSalesManagerDbContext;
		}
		...
		public T Update(T entity)
		{
            ...
            _context.Context.VPlusSalesManagerDbContext.Entry(entity).State = EntityState.Modified;
            return updated;
        ...
        public IEnumerable<T> UpdateRange(List<T> entities)
        {
            ...
            foreach (var item in entities)
            {
                ...
                _context.Context.VPlusSalesManagerDbContext.Entry(item).State = EntityState.Modified;
                retVals.Add(updated);
            }
            return retVals;
        }
